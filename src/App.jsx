    ...
    if (taskRes.data) {
      ...
      setTasks(taskRes.data.filter(t => !clearedIds.has(t.id) && !t.cleared_at).map(t => {
        ...
        return {
          ...
          // Rai task generator fields. createdBy === 'rai' tags this row as
          // generated by Rai's daily sweep, not entered by the user. UI uses
          // it for the sparkle+pill badge and for filtering when "Rai Tasks"
          // toggle is Off. raiDismissedAt nonzero means user dismissed it.
          createdBy: t.created_by || 'user',
          raiPattern: t.rai_pattern || null,
          raiDismissedAt: t.rai_dismissed_at || null,
    ...
    <div className="app-root" style={{ minHeight: "100vh", fontFamily: "'Manrope', system-ui, sans-serif", color: C.text, background: C.bg }}>
      ...
      <div className="r-main">
        ...
        {page === "today" && (() => {
          ...
          // ─── DATA PREP ───────────────────────────────────────────────────
          // Visible tasks = non-dismissed (locally) AND non-Rai-dismissed (server-side
          // soft-dismissal flag). When "Rai Tasks" toggle is Off, also hide any task
          // created by Rai — they stay in the DB but vanish from the list. Flip toggle
          // back On and they reappear.
          const taskGenOff = raiState && raiState.task_gen_enabled === false;
          const visibleTasks = tasks.filter(t => {
            if (dismissedIds[t.id]) return false;
            if (t.createdBy === 'rai') {
              if (t.raiDismissedAt) return false;
              if (taskGenOff) return false;
            }
            return true;
          });
                          ...
                          <div
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: 14, fontWeight: 600, color: C.text, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {t.createdBy === 'rai' && (
                                  <span
                                    title={t.raiPattern ? `Rai detected: ${t.raiPattern}` : "Created by Rai"}
                                    style={{
                                      display: "inline-flex",
                                      alignItems: "center",
                                      gap: 3,
                                  ...
                                  >
                                    {/* Sparkle icon — small, same color as text */}
